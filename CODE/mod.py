...
def Sentiment():  
    ...
    dfx=dff[['type','release_year','description']]
    ...
    dfx1=dfx.groupby(['type','Sentiment']).size().reset_index(name='Total Content')
    dfx=dfx1[dfx['Release Year']>=2010]
    dfx=dfx[dfx['type']=="Movie"]
    fig4 = px.bar(dfx, x="type", y="Total Content", color="Sentiment", title="Sentiment of content on Netflix")
    fig4.show()
